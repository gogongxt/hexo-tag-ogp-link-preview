...
function getTimeout(linkPreviewCfg, defaultValue) {
    return getObjectValueFrom(linkPreviewCfg, 'timeout', 'number', defaultValue);
}

function getEnable(linkPreviewCfg, defaultValue) {
    if (hasTypeOfProperty(linkPreviewCfg, 'enable', 'boolean')) {
        return linkPreviewCfg.enable;
    }
    return defaultValue;
}

function getSimpleLink(linkPreviewCfg, defaultValue) {
    if (hasTypeOfProperty(linkPreviewCfg, 'simple_link', 'boolean')) {
        return linkPreviewCfg.simple_link;
    }
    return defaultValue;
}
...
module.exports = (hexoCfg) => {
    ...
    return {
        ...
        timeout: getTimeout(linkPreviewCfg, TIMEOUT),
        enable: getEnable(linkPreviewCfg, ENABLE),
        simple_link: getSimpleLink(linkPreviewCfg, SIMPLE_LINK),
    };
};
