...
module.exports = (args, config) => {
    ...
    const { class_name: className, description_length: descriptionLength, disguise_crawler: isCrawler, timeout } = config;
    const fetchOptions = getFetchOptions(isCrawler);

    return {
        scrape: { url, fetchOptions, timeout },
        generate: { target, rel, loading, classSuffix, descriptionLength, className },
    };
};
